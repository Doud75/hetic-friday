...
# Resolve database host (prefer explicit host to avoid gcloud dependency)
# - ALLOYDB_PRIMARY_IP: explicit AlloyDB primary IP
# - RDS_HOST: explicit PostgreSQL host (e.g., AWS RDS)
if [ -z "${ALLOYDB_PRIMARY_IP}" ]; then
	if [ -n "${RDS_HOST}" ]; then
		ALLOYDB_PRIMARY_IP="${RDS_HOST}"
	else
		# Fetch the primary IP from AlloyDB (requires gcloud)
