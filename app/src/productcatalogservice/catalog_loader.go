...
func loadCatalogFromPostgres(catalog *pb.ListProductsResponse) error {
	...
	user := getEnv("RDS_USER", "postgres")
	password := os.Getenv("RDS_PASSWORD")
	dbName := os.Getenv("RDS_DB_NAME")
	tableName := os.Getenv("RDS_TABLE_NAME")
	sslmode := getEnv("RDS_SSLMODE", "require")

	if host == "" || dbName == "" || tableName == "" {
		return fmt.Errorf("missing required RDS settings: RDS_HOST, RDS_DB_NAME, RDS_TABLE_NAME")
	}
